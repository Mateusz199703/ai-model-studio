...
.upzone:hover{border-color:rgba(201,169,110,0.7);background:rgba(201,169,110,0.07)}
...
<div class="lpanel">
  <div class="sec">
    ...
    <div class="row">
      ...
      <button class="iBtn" id="toggleKey">
  ...
  <div class="sec" id="uploadSec">
    ...
    <div id="presetWrap" style="margin-top:10px">
      ...
      <div class="pmodels" id="presetGrid">
        <div class="pmod" data-url="https://storage.googleapis.com/falserverless/model_tests/leffa/person_image.jpg"><span>Modelka 1</span></div>
        <div class="pmod" data-url="https://images.unsplash.com/photo-1529139574466-a303027f1d1f?w=400&q=80"><span>Modelka 2</span></div>
        <div class="pmod" data-url="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=400&q=80"><span>Modelka 3</span></div>
        <div class="pmod" data-url="https://images.unsplash.com/photo-1524504388940-b1c1722653e1?w=400&q=80"><span>Modelka 4</span></div>
      </div>
    </div>
  </div>
...
document.querySelectorAll('.pcard').forEach(function(card){
  card.addEventListener('click',function(){
    ...
    activeEndpoint=card.dataset.endpoint;
...
document.getElementById('customEndpoint').addEventListener('input',function(){
  ...
  document.querySelectorAll('.pcard').forEach(function(c){c.classList.remove('on');});
  activeEndpoint=v;
  if(v.indexOf('tryon')!==-1)activeType='tryon';
  else if(v.indexOf('edit')!==-1||v.indexOf('nano-banana')!==-1||v.indexOf('gemini')!==-1||v.indexOf('image-to-image')!==-1)activeType='edit';
  else activeType='gen';
  updateUI();
});
...
document.getElementById('mFile').addEventListener('change',function(){
  if(this.files&&this.files[0])loadFile(this.files[0],'m');
});
...
document.getElementById('delP').addEventListener('click',function(){
  ...
  document.getElementById('pPrev').classList.remove('on');
...
document.getElementById('delM').addEventListener('click',function(){
  ...
  document.getElementById('mPrev').classList.remove('on');
  document.getElementById('mZone').style.display='';
...
function pollQueue(key,endpoint,reqId){
  var start=Date.now();
  var attempts=0;
  function attempt(){
